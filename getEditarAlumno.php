<div>
    ...
    $alumno = $conn->getAlumnoPorId($idAlumno);
    session_start();
    ?>
    <h2>Agregar Carrera</h2>
        <form id="formulario">
            <label for="nombres">Nombres : *</label>
            <input type="text" name = "nombres" id="txtNombre" value = "<?php echo $alumno['nombre']?>"/></br>
            <label for="apPaterno">Ap. Paterno : *</label>
            <input type="text" name = "apPaterno" id="txtApPaterno" value = "<?php echo $alumno['apPaterno']?>"/></br>
            <label for="apMaterno">Ap. Materno : *</label>
            <input type="text" name = "apMaterno" id="txtApMaterno" value = "<?php echo $alumno['apMaterno']?>"/></br>
            <label for="correo">Correo : </label>
            <input type="text" name = "correo" id="txtCorreo" value = "<?php echo $alumno['correo']?>"/></br>
            <label for="noControl">Número de control : *</label>
            <input type="text" name = "noControl" id="txtNoControl" value = "<?php echo $alumno['noControl']?>"/></br>
            <label for="nip">NIP : *</label>
            <input type="password" name = "nip" id="txtNip" value = "<?php echo $alumno['nip']?>"/></br>
            <label for="nipComp">NIP (de nuevo) : *</label>
            <input type="password" name = "nipComp" id="txtNipComp" value = "<?php echo $alumno['nip']?>"/></br>
            <label for="telefono">Teléfono : </label>
            <input type="text" name = "telefono" id="txtTelefono" value = "<?php echo $alumno['telefono']?>"/></br>
            <label for="ciudad">Ciudad : </label>
            <input type="text" name = "ciudad" id="txtCiudad" value = "<?php echo $alumno['ciudad']?>"/></br>
            <label for="domicilio">Domicilio : </label>
            <input type="text" name = "domicilio" id="txtDomicilio" value = "<?php echo $alumno['domicilio']?>"/></br>
            <label>
                Carrera : *
            </label>
            <select id="selectCarreras">
                <?php 
                    foreach ($carreras as $carrera) {
                        if($carrera['id'] == $alumno['idCarrera']){
            ...
            <select id = "selectGrupos">
                ...
                <?php 
                    foreach ($grupos as $grupo) {
                        if($grupo['id'] == $alumno['idGrupo']){
                            echo '<option value= "'.$grupo['id'].'" selected>';    
                        }else{
            ...
            <input type="text" name = "nombresTutor" id="txtNombreTutor" value = "<?php echo $alumno['nombreTutor']?>"/></br>
            <label for="domicilioTutor">Domicilio tutor : </label>
            <input type="text" name = "domicilioTutor" id="txtDomicilioTutor" value = "<?php echo $alumno['domicilioTutor']?>"/></br>
            <label for="telefonoTutor">Teléfono Tutor : </label>
            <input type="text" name = "telefonoTutor" id="txtTelefonoTutor" value = "<?php echo $alumno['telefonoTutor']?>"/></br>
            <label for="ciudadTutor">Ciudad Tutor : </label>
            <input type="text" name = "ciudadTutor" id="txtCiudadTutor" value = "<?php echo $alumno['ciudadTutor']?>"/></br>
    ...
    <script>
        var alumno = <?php echo json_encode($alumno); ?>;
                ...
                if(idCarrera>0){
                    ...
                    if(idGrupo>0){
                        if(nip.val().localeCompare(nipComp.val()) == 0){
                            ...
                            $.ajax({
                                method: "POST",
                                url: "Conexiones/Alumnos/editarAlumno.php",
                                data: {nombre: nombre.val(), apPaterno: apPaterno.val(), apMaterno: apMaterno.val(), 
                                    ...
                                    idCarrera: idCarrera, idGrupo: idGrupo, noControlAnterior: alumno['noControl'], nipAnterior: alumno['nip'],
                                    idAlumno: alumno['id']}
                            }).done(function (msg) {
                                alert(msg);
