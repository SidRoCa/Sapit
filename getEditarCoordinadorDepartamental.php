<div>
    ...
    $coordinadorDepartamental = $conn->getCoordinadorDepartamentalPorId($idCoordinador);
    $departamentos = $conn->getListaDepartamentos();
    ?>
    <h2>Editar Coordinador Departamental</h2>
    <form id="formulario">
        <label for="nombres">Nombre Completo: *</label>
        <input type="text" name = "nombre" id="txtNombre" value="<?php echo($coordinadorDepartamental['nombre']) ?>"/></br>
        ...
        <select id="selectDepartamento">
            ...
            foreach ($departamentos as $departamento) {
                ...
                if ($departamento['id'] == $coordinadorDepartamental['iddpto']) {
        ...
        <input type="text" name = "usuario" id="txtUsuario" value="<?php echo($coordinadorDepartamental['usuario']) ?>"/></br>
        <label for = "contraseña">
            Contraseña : *
        </label>
        <input type="password" name = "contraseña" id="txtContraseña" value="<?php echo($coordinadorDepartamental['password']) ?>" /></br>
        <label for = "contraseñaComp">
            Contraseña (de nuevo) : *
        </label>
        <input type="password" name = "contraseñaComp" id="txtContraseñaComp" value="<?php echo($coordinadorDepartamental['password']) ?>" /></br>
    ...
    <script>
        ...
        function guardar() {
            ...
            var idCoordinadorDepartamental = <?php echo($idCoordinador)?>;
            if (!nombre.val() || !usuario.val() || !contraseña.val() || !contraseñaComp.val() ){ 
                ...
                if (contraseña.val().localeCompare(contraseñaComp.val()) == 0) {
                    ...
                    if (idDepartamento > 0) {
                        ...
                        $.ajax({
                            method: "POST",
                            url: "Conexiones/CoordinadoresDepartamentales/editarCoordinadorDepartamental.php",
                            data: {idCoordinador: idCoordinadorDepartamental, nombre: nombre.val(), usuario: usuario.val(), contraseña: contraseña.val(), idDepartamento: idDepartamento}
                        }).done(function (msg) {
